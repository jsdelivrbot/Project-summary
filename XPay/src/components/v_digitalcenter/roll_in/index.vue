<template>
    <div class="backPadding boxWhite">
        <top-bar :back-type="2">
            <span slot="content">{{'转入' | translate}}</span>
        </top-bar>
        <div class="m__wrap_30 p__b_80">
            ...
            <div class="formItem__wrap">
                ...
                <div class="formItem">
                    <input :class="{error:!requestData.amount && hasValidate}" v-model="requestData.amount"  :placeholder="'请输入转入的Xpay数量' | translate" type="text" class="inputItem">
                    <div class="formItem_border"></div>
                </div>
            </div>
            <div class="tips">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submit(){
        $mask("提交请求中");
        server.xBaoTransferIn(this.requestData).then(res =>{
          this.$toast.clear();
          this.initData();
          this.$toast($t('资产转入成功'));
        }).catch(error =>{
