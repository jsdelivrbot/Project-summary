<template>
  ...
  <div class="container marTop330 network">
    <div class="com-div-fr">
      ...
      <div class="network-filter">
        ...
        <div class="filter-ar">
          ...
          <a href="javascript:;" class="btn com-btn purple-btn" @click="left_bottom">
                        {{'左区底层'| translate}}
                      </a>
          <a href="javascript:;" class="btn com-btn purple-btn" @click="right_bottom">
                        {{'右区底层'| translate}}
                      </a> -->
        </div>
      </div>
    ...
    <div class="com-div-fr network-map" id="network_jd">
      <net-node ref="net_child" :ndata="ndata" :type="net_type"> </net-node>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    // 保存顶层的userCode
    // this.axios.all([
    //   this.axios.post("/api/user/GetTJTreeData", {
    ...
    //   this.idData.top = res1.Data[0].UserCode;
    //   this.idData.left = res2.Data[0].UserCode;
    //   this.idData.right = res3.Data[0].UserCode;
    // }));
  },
  methods: {
    tocheck() {
    ...
    getRootNode(Tree, item, userCode) {
      ...
      if (item.UserCode == userCode) {
        Tree.tree = {
          ...
          Total: item.Total,
          child: [],
          JDLeftChildCount: item.JDLeftChildCount,
          JDRightChildCount: item.JDRightChildCount
        };
      }
      nodeTree = Tree.tree;
      return nodeTree;
    ...
    getNextNode(Tree, item) {
      var newTree = Tree;
      var posNum;
      if (item.JDUserCode == Tree.UserCode) {
        var pos = item.JDPosition > 1 ? Number(item.JDPosition) - 1 : 0;
    ...
    constructTree(newTree, item) {
      ...
      if (item.UserCode == "hy4225609") {
      }
      if (new_Tree.child.length > 0) {
        for (var i = 0; i < new_Tree.child.length; i++) {
          var pos =
            new_Tree.child[i].JDPosition > 1
              ? Number(new_Tree.child[i].JDPosition) - 1
              : 0;
          var new_child_tree = this.getNextNode(new_Tree.child[i], item);
          // 判断第三层存在吗， 存在则给第三层添加child
          if (new_Tree.child[i].child) {
            for (var k = 0; k < new_Tree.child[i].child.length; k++) {
              var kItem = new_Tree.child[i].child[k];
              kItem = this.getNextNode(kItem, item);
            }
          }
    ...
    Upper_story() {
      this.params["isTop"] = "";
      this.params["jdposition"] = "";
      this.userCode = this.nodeTree ? this.nodeTree.JDUserCode : this.userCode;
      this.params.userCode = this.userCode;
      this.getNetworkData();
    },
    //顶层
    topper_story() {
      ...
      this.params["jdposition"] = "";
      this.userCode = this.idData.top;
      this.params.userCode = this.idData.top;
    ...
    left_bottom() {
      this.params["isTop"] = "";
      this.params["jdposition"] = 1;

      // if (this.ndata.child) {
      //   var postUserCode = this.ndata.child[0].UserCode;
      // }
      this.userCode = this.idData.left;
      this.params.userCode = this.userCode;
    ...
    right_bottom() {
      // if (this.ndata.child) {
      //   var postUserCode = this.ndata.child[1].UserCode;
      // }
      this.params["isTop"] = "";
      this.params["jdposition"] = 2;

      this.userCode = this.idData.right;
      this.params.userCode = this.idData.right;
