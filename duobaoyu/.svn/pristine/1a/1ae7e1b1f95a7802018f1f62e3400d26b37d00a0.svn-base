<template>
  <div class="img1_box" v-if="PondForm.popShow" :class="divClass" @click="btnClick(item)">
  ...
  <div class="img1_box" v-else :class="divClass">
    <img v-if="item.PrizeStatus==3" class="qipao" src="static/imgs/qipao.png" >
...
<script>
  ...
  export default {
    props: ["item", "PondForm", "bindFun", "openPop"],
    data() {
      return {
        divClass: {
          img1_box: false,
    ...
    created() {
      this.changePond();
      //console.log('bubble',this.bubble)
    },
    watch: {
    },
    computed: {
